...
const getBestColor = colors => {
  ...
  for (const [_, color] of Object.entries(colors)) {
    ...
    const deltaHue = (hueDistance < 0.1 && hsl[1] < 0.85 || hsl[2] > 0.7 ? 0.01/hueDistance : 0) * dHueMult;
    ...
    const distance = Math.sqrt(deltaSaturation * deltaSaturation + deltaLightness * deltaLightness + deltaHue * deltaHue  );
