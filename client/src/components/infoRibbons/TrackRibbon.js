...
const getTimeStuff = (rawDate, rawDuration) => {
  ...
  if (daysSince < 10) {
    if (daysSince < 1) dateAddedtext = 'Less than 1 day ago';
    else dateAddedtext = `${daysSince} day${daysSince > 1 ? 's' : ''} ago`;
  } else dateAddedtext = dateAdded.toLocaleString(DateTime.DATE_MED);
  ...
  durationText = `${durationHr > 0 ? `${durationHr}:` : ''}${durationMin}:${
...
export const TrackRibbon = memo(({ data, index, style }) => {
  ...
  if (!data) return null;
  const { dateAddedtext, durationText } = getTimeStuff(
    data.added_at,
