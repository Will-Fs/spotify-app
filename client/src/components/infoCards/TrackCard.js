...
export const getTrackCardData = async (props) => {
  ...
  if (isTopTrack) {
    if (timeFrame === 'short') secondLabel = 'Top track this month';
    else if (timeFrame === 'medium') secondLabel = 'Top track past 6 months';
    else {
  ...
  let secondLabelHeader = !isTopTrack ? (
    <p className="second-card-label">{secondLabel}</p>
  ) : null;
  ...
  if (props.object === true) {
    ...
    if (totalTime > 36000) return `about ${Math.round(totalTime)} hr`;
    ...
    const durationText = `${hours > 0 ? `${hours}:` : ''}${
      minutes > 0 ? `${minutes}:` : ''
    }${seconds}`;

    secondLabelHeader = (
      <p className="second-card-label">
        <b
          style={{
        ...
        {`${data.artists[0].name} • ${data.album.release_date.substring(
          0,
          4
        )} • ${durationText}`}
      </p>
