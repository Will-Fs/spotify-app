...
export const getPlaylistCardData = async (props) => {
  ...
  const followers =
    data.followers.total > 0
      ? `${formatter.format(data.followers.total)} like${
  ...
  if (needsOwner) {
    ...
    secondLabelHeader = (
      <p className="second-card-label" style={{ color: color.secondaryColor }}>
        {followers}
      </p>
    );
  } else {
  ...
  if (props.object === true) {
    ...
    secondLabelHeader = (
      <p
        className="second-card-label"
        style={{ color: color.secondaryColor }}
      >{`${data.owner.display_name}${followers ? ` • ${followers}` : ''} • ${
        props.numTracks
      } songs, ${props.totalTime}`}</p>
    );
    type = 'playlist';
  }
