...
export const InfoCardRender = (data) => {
  ...
  const bgImage =
    data.object !== true
      ? cardBackground(data.color.bottomColor, data.color.topColor)
      : null;
  return (
    <div
      className={`card-large ${
        data.object === true ? `card-object ${data.type ?? ''}` : ''
      }`}
    ...
    >
      ...
      <img
        ...
        style={{
          boxShadow:
            data.type !== 'me' && data.object === true
              ? '0 0 10vw 0px rgba(var(--body-bg-color-value), 0.5)'
              : null,
        }}
      />
      <div className="profile-stats">
        ...
        {data.firstLabel ? (
          <h2
            className="first-card-label "
            style={{ color: data.color.secondaryColor }}
            dangerouslySetInnerHTML={{ __html: data.firstLabel }}
          ></h2>
        ) : null}
        ...
        {data.titleHeader || data.type === 'me' ? null : (
          <div className="color-swatches">
            {data.color.allColors.map((color) => (
              <ColorSwatch colorData={color} key={uuid()} />
            ))}
          </div>
        )}
      </div>
